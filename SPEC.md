...
**Burn-window scoping:** unless stated otherwise, every metric below is computed using only the data points between ignition and burnout (the "burn window" — see `getBurnWindow` in the reference implementation), not the full `trimRange`. A trim range may legitimately include a few seconds of padding before ignition and after burnout for visual/contextual purposes (see Auto-trimming below); that padding must never be included in any metric's calculation. This applies equally to metrics not yet implemented (Action Time, Specific Impulse, Peak/Average Chamber Pressure, Thrust Coefficient, Mass Flow Rate) once they are built.

- **Total Impulse** = ∫ Thrust dt (numerical integration, e.g., trapezoidal rule) over the burn window
- **Peak Thrust** = max(Thrust) over the burn window
...
- **Peak / Average Chamber Pressure** = max / mean of recorded pressure channel over the burn window, if present
- **Thrust Coefficient (Cf)** = Thrust ÷ (Chamber Pressure × Nozzle Throat Area), evaluated over the burn window — requires nozzle throat area as a known input
...
**Trim precedence:** when a test has a `trimRange` set (V1, manual trim/crop tool), all data outside `trimRange` is excluded entirely from that test — not read by any calculation, plot, or averaging step. The burn window (used for the actual metric calculations above) is then determined from within that trimmed data, per "Burn-window scoping" above.
