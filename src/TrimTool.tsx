...
export default function TrimTool({ test, initialRange, onConfirm }: TrimToolProps) {
  ...
  const [start, setStart] = useState(initialRange?.start ?? dataMin)
  const [end, setEnd] = useState(initialRange?.end ?? dataMax)
  ...
  // Deterministic +/- stepping, independent of the browser's native number
  // spinner (which was unreliable when paired with dynamic min/max bounds).
  function nudgeStart(delta: number) {
    setStart((prev) => Math.max(dataMin, Math.min(end, +(prev + delta).toFixed(3))))
  }
  function nudgeEnd(delta: number) {
    setEnd((prev) => Math.max(start, Math.min(dataMax, +(prev + delta).toFixed(3))))
  }
  ...
  return (
    <div className="trim-tool">
      <p className="import-error" style={{ color: '#ffcf6e' }}>
        {crossingCount > 1
          ? `Detected thrust rises above 5% of peak ${crossingCount} times — this usually means a pre-ignition spike or other anomaly. Drag the green lines to include only the real burn. Drag the green lines to adjust the trim range, or confirm to use the full recording.`
      ...
      <div className="mapping-row">
        ...
        <div className="stepper">
          <button type="button" className="step-btn" onClick={() => nudgeStart(-1)}>−</button>
          <input
            type="number"
            value={start}
            onChange={(e) => {
      ...
      <div className="mapping-row">
        <span>End (s)</span>
        <div className="stepper">
          <button type="button" className="step-btn" onClick={() => nudgeEnd(-1)}>−</button>
          <input
            type="number"
            value={end}
            onChange={(e) => {
