...
// Slices time/thrust down to exactly the ignition-to-burnout burn window
// (using sustained-threshold ignition and simple last-above-threshold
// burnout). This is distinct from the caller's trim range — trimming
// controls what data EXISTS for the test (may include a few seconds of
// padding for visual context); this window controls what data actually
// COUNTS for burn-specific metrics like Total Impulse and Peak Thrust.
export function getBurnWindow(
  time: number[],
  thrust: number[],
  thresholdRatio = 0.05,
  minDurationSeconds = 0.05
): { time: number[]; thrust: number[]; ignition: number; burnout: number } | null {
  const ignition = findSustainedIgnition(time, thrust, thresholdRatio, minDurationSeconds)
  ...
  if (burnoutIdx === -1) return null
  const burnout = time[burnoutIdx]

  const windowTime: number[] = []
  const windowThrust: number[] = []
  for (let i = 0; i < time.length; i++) {
    if (time[i] >= ignition && time[i] <= burnout) {
      windowTime.push(time[i])
      windowThrust.push(thrust[i])
    }
  }

  if (windowTime.length < 2) return null

  return { time: windowTime, thrust: windowThrust, ignition, burnout }
}

// Computes all V1 core metrics for a test's TRIMMED data (per SPEC.md §5
// "Trim precedence"). Total Impulse and Peak Thrust are further restricted
// to the ignition-to-burnout burn window within that trimmed data — see
// getBurnWindow() — so padding seconds included by a trim do not skew them.
...
export function computeMetrics(time: number[], thrust: number[]): TestMetrics | null {
  ...
  const burnWindow = getBurnWindow(time, thrust)
  if (!burnWindow) return null

  const { time: burnWindowTime, thrust: burnWindowThrust, ignition, burnout } = burnWindow

  const peakThrust = Math.max(...burnWindowThrust)
  const totalImpulse = trapezoidalIntegral(burnWindowTime, burnWindowThrust)
  const burnTime = burnout - ignition
